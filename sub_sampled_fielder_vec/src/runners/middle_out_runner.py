...
def save_progress_checkpoint(run_dir: str, results: List[Dict], p_values: List[float],
    ...
    for i, result in enumerate(results):
        if result is None:
            ...
            checkpoint['results'].append({
                ...
                'sigma2_avg_M': result.get('sigma2_avg_M', float('nan')),
                'sigma2_avg_S': result.get('sigma2_avg_S', float('nan'))
            })
    ...
    log_info('middle_out', f"  💾 Checkpoint saved: {checkpoint_path}")
...
def sweep_for_params_middle_out(
...
) -> Tuple[List[float], List[float], List[float], List[float], Dict[str, List[Tuple[float, float, float]]], float, List[float], List[float],
    ...
    """
    # Compute reference partition quality (if not already done in bootstrap_sweep)
    ...
    if partition_ref is None:
        try:
            ...
            log_info('middle_out', f"Reference partition quality (σ₂): {reference_partition_quality:.6f}")
        except Exception as e:
            log_warning('middle_out', f"Failed to compute reference partition: {e}")
            partition_ref = None
            reference_partition_quality = float('nan')
            reference_partition_split = None
    else:
        # partition_ref already computed, extract quality from it
        from spectraltree.spectral_tree_reconstruction import svd2
        try:
            s_sliced = M[partition_ref, :]
            s_sliced = s_sliced[:, ~partition_ref]
            reference_partition_quality = float(svd2(s_sliced))
            # Compute split sizes if not provided
            if reference_partition_split is None:
                n_true = int(np.sum(partition_ref))
                n_false = len(partition_ref) - n_true
                reference_partition_split = (min(n_true, n_false), max(n_true, n_false))
            log_info('middle_out', f"Reference partition quality (σ₂): {reference_partition_quality:.6f}")
        except Exception as e:
            log_warning('middle_out', f"Failed to compute reference partition quality: {e}")
            reference_partition_quality = float('nan')
    ...
    log_info('middle_out', f"Starting middle-out processing: {n_p_values} p-values, middle_idx={middle_idx}")
    log_info('middle_out', f"Progress checkpoints will be saved to: {run_dir}/progress_checkpoint.json")
    ...
    log_info('middle_out', f"  This will take ~{cfg.experiment.bootstrap_reps * 6}s for {cfg.experiment.bootstrap_reps} bootstrap reps...")

    round_start_time = time.time()
    results[middle_idx] = process_p_value_worker(M, fiedler_ref, partition_ref, L_M, p_values[middle_idx], cfg, reference_partition_split)
    elapsed = time.time() - round_start_time

    log_info('middle_out',
             f"Middle result (took {elapsed:.1f}s): {cfg.guardrails.metric}={results[middle_idx][cfg.guardrails.metric]:.2f}%",
             force=True)
    ...
    workers_per_side = cfg.experiment.num_workers // 2  # Split workers between directions
    offset = 1
    round_num = 1

    with multiprocessing.Pool(cfg.experiment.num_workers) as pool:
        while not (high_complete and low_complete):
            to_process = []  # List of (index, direction, p_value)

            log_info('middle_out', f"\nRound {round_num}: offset={offset}")
            ...
            if not high_complete:
                ...
                if high_indices:
                    log_info('middle_out', f"  High side: indices {high_indices} (p={[p_values[i] for i in high_indices]})")
                    to_process.extend([(i, 'high', p_values[i]) for i in high_indices])
                else:
                    log_info('middle_out', "  High side: no more p-values")
                    high_complete = True
            ...
            if not low_complete:
                ...
                if low_indices:
                    log_info('middle_out', f"  Low side: indices {low_indices} (p={[p_values[i] for i in low_indices]})")
                    to_process.extend([(i, 'low', p_values[i]) for i in low_indices])
                else:
                    log_info('middle_out', "  Low side: no more p-values")
                    low_complete = True

            if not to_process:
                log_info('middle_out', "No more work, breaking")
                break

            # Process chunk in parallel
            log_info('middle_out', f"  Processing {len(to_process)} p-values in parallel...")

            round_start_time = time.time()
            worker_args = [(M, fiedler_ref, partition_ref, L_M, p, cfg, reference_partition_split) for (_, _, p) in to_process]
            chunk_results = pool.starmap(process_p_value_worker, worker_args)
            elapsed = time.time() - round_start_time

            log_info('middle_out', f"  ✓ Completed in {elapsed:.1f}s")

            # Store results
            for (idx, direction, p), result in zip(to_process, chunk_results):
                results[idx] = result
                log_info('middle_out',
                        f"    p[{idx}]={p:.4g}: {cfg.guardrails.metric}={result[cfg.guardrails.metric]:.2f}%")
            ...
            if not high_complete:
                ...
                if check_guardrails_high(high_results_so_far, cfg.guardrails.metric):
                    ...
                    for i in range(middle_idx + 1, n_p_values):
                        if results[i] is None:
                            results[i] = create_guardrail_high_result(p_values[i], M_constants, reference_partition_quality, reference_partition_split)
                            log_info('middle_out', f"    Filled p[{i}]={p_values[i]:.4g} with 100%")
            ...
            if not low_complete:
                ...
                if check_guardrails_low(low_results_so_far, cfg.guardrails.metric, cfg.guardrails.low_side_threshold, cfg.guardrails.low_side_epsilon):
                    effective_threshold = cfg.guardrails.low_side_threshold + cfg.guardrails.low_side_epsilon
                    ...
                    for i in range(middle_idx - 1, -1, -1):
                        if results[i] is None:
                            ...
                            log_info('middle_out', f"    Filled p[{i}]={p_values[i]:.4g} with 50% (below threshold)")

            # If one side is complete, reallocate workers to the other side
            if high_complete and not low_complete:
                workers_per_side = cfg.experiment.num_workers  # All workers to low side
                log_info('middle_out', "  Reallocating all workers to low side")
            elif low_complete and not high_complete:
                workers_per_side = cfg.experiment.num_workers  # All workers to high side
                log_info('middle_out', "  Reallocating all workers to high side")

            offset += workers_per_side if not (high_complete and low_complete) else cfg.experiment.num_workers // 2
            round_num += 1
    ...
    sigma2_avg_M_list = []
    sigma2_avg_S_list = []
    partition_split_M_list = []
    partition_split_S_list = []
    result_source_list = []

    # Initialize metrics_dict from first non-None result that has metrics
    metrics_dict = {}
    for result in results:
        if result is not None and 'metrics' in result and result['metrics']:
            metrics_dict = {key: [] for key in result['metrics'].keys()}
            break
    if not metrics_dict:
        # Fallback: use default metric keys if no results have metrics yet
        metric_keys = [
            'operator_norm_error',
    ...
    for i, result in enumerate(results):
        if result is None:
            ...
            sigma2_avg_M_list.append(float('nan'))
            sigma2_avg_S_list.append(float('nan'))
            partition_split_M_list.append(None)
            partition_split_S_list.append(None)
            result_source_list.append('error')
            ...
            sigma2_avg_M_list.append(result.get('sigma2_avg_M', float('nan')))
            sigma2_avg_S_list.append(result.get('sigma2_avg_S', float('nan')))
            partition_split_M_list.append(result.get('partition_split_M'))
            partition_split_S_list.append(result.get('partition_split_S'))
            result_source_list.append(result.get('result_source', 'unknown'))
