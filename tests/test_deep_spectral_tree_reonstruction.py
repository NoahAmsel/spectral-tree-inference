...
#     x.length = 1
merging_method_list = ['angle']
#merging_method_list = []
RF = {'least_square': [], 'angle': [], 'tensor': []}
F1 = {'least_square': [], 'angle': [], 'tensor': []}
for i in range(num_itr):
    ...
    for merge_method in merging_method_list:
        ...
        tree_rec = spectral_method.deep_spectral_tree_reonstruction(observations, reconstruct_tree.JC_similarity_matrix, 
            taxon_namespace = reference_tree.taxon_namespace, threshhold = 130,merge_method = merge_method)
