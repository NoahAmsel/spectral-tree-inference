...
def comp_scorers(m, Ns, k, scorers, n_trees=3, obs_per_tree=3, proba_bounds=(0.75, 0.95)):
    # Ns is array of n
    transition_maker = NoahClade.NoahClade.gen_symmetric_transition
    stats = []

    itr = 1
    total_iters = n_trees*obs_per_tree*len(Ns)*len(scorers)
        ...
        for _ in range(obs_per_tree):
            for n in Ns:
                ...
                for scorer in scorers:
                    ...
                    F1, precision, recall = NoahClade.tree_Fscore(inferred_tree, ref_tree, verbose=True)
                    correct = 1 if (F1==1) else 0
                    stats.append({"n": n, "scorer": scorer.__name__[6:], "F1 %":100*F1, "precision %":100*precision, "recall %":100*recall, "correct %": 100*correct})
                    itr += 1
    return stats
...
if __name__ == "__main__":
    ...
    sns.catplot(data=stats, x="scorer", y="F1 %", inner="stick", col="n", bw=.8, scale="count", kind="violin", col_wrap=3)
    #plt.show()
