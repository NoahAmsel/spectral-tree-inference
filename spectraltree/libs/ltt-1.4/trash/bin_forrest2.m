...
if isfield(opt, 'onlymodel') && opt.onlymodel == 1
  onlymodel = 1;
  t_given = opt.t;        % the given tree structure
  % note that we do not need the list of roots
else
  onlymodel = 0;
end
...
while 1
  ...
  if ~onlymodel
    ...
    % we are already given some tree structure!
  end
